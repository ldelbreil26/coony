...
export default function PageConteneurMiniJeu() {
  ...
  useEffect(() => {
    const chargerInfo = async () => {
      const info = await getMiniJeuById(id);
      setJeuInfo(info);
    };
    chargerInfo();
  }, [id]);
  ...
  const quitterJeu = () => {
    // On retourne au dashboard en "replace" pour réinitialiser la pile de navigation
    router.replace('/tableau_de_bord_enfant'); 
  };

  const renderGame = () => {
    // On convertit l'ID en string au cas où
    switch (id.toString()) {
      case '1':
  ...
  return (
    <View style={styles.mainContainer}>
      {/* Bouton de sortie flottant - Toujours au premier plan */}
      <TouchableOpacity 
...
const styles = StyleSheet.create({
  mainContainer: {
    flex: 1,
    backgroundColor: COLORS.background, // On utilise ton design system
  },
  boutonFermer: {
    position: 'absolute',
    top: 50, 
    right: 20,
    zIndex: 9999, // Très élevé pour rester au-dessus des animations
    backgroundColor: 'white',
    borderRadius: 30,
    // Petit effet d'ombre pour qu'il se détache du jeu
    shadowColor: "#000",
    shadowOpacity: 0.1,
    shadowRadius: 5,
    elevation: 5
